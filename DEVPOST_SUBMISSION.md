...
The live workflow uses GPT‑5.6 Sol at high reasoning effort, Programmatic Tool Calling, an OpenAI-hosted shell with networking disabled, explicit prompt caching, structured outputs, and `previous_response_id` with `reasoning.context: "all_turns"`. The server derives baseline and counterexample status from actual shell-call output rather than model prose. A bundled reference replay is clearly labeled and used only when live access is unavailable.
